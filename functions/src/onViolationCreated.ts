...
export const onViolationCreated = onDocumentUpdated('violations/{violationId}', async (event) => {
  ...
  try {
    ...
    if (wasLowOrNeedsReview && isNowHighOrCritical) {
      ...
      const assetRef = db.collection('assets').doc(violation.asset_id);
      ...
      const orgRef = db.collection('organizations').doc(ownerOrgId);
      ...
      if (alertThreshold === 'CRITICAL' && violation.severity === 'CRITICAL') shouldSend = true;
      if (alertThreshold === 'HIGH' && (violation.severity === 'CRITICAL' || violation.severity === 'HIGH')) shouldSend = true;
