...
export function FilterTabs({ tabs, active, onChange, className }: FilterTabsProps) {
  return (
    <div className={clsx('flex items-center gap-1 p-1 bg-zinc-100 rounded-xl w-fit', className)}>
      {tabs.map((tab) => (
        <button
          key={tab.key}
        ...
        >
          ...
          {tab.count !== undefined && (
            <span className={clsx(
              ...
              active === tab.key ? 'bg-zinc-100 text-brand-text' : 'bg-zinc-200 text-zinc-400',
            )}>
              {tab.count}
            </span>
          )}
