                ...
                {RIGHTS_TIERS.map((tier) => {
                  ...
                  return (
                    <button
                      ...
                      className={`relative p-5 rounded-2xl border-2 text-left transition-all duration-300 group hover:shadow-soft-lg ${isSelected
                    ...
                    >
                      <div className="flex flex-col h-full gap-4">
                        <div className={`p-2.5 rounded-xl w-fit transition-transform duration-300 group-hover:scale-110 ${isSelected ? 'bg-brand-text text-white dark:text-black' : `${tier.bg} ${tier.color}`
                          }`}>
                          <Icon className="w-5 h-5" />
                        </div>

                        <div className="space-y-1">
                          <p className={`text-[11px] font-black uppercase tracking-tight ${isSelected ? 'text-brand-text' : 'text-brand-text/70'
                            }`}>{tier.label}</p>
                          <p className={`text-[10px] font-bold ${isSelected ? 'text-brand-text/60' : 'text-brand-muted'
                            }`}>{tier.description}</p>
                        </div>

                        <p className={`text-[10px] leading-relaxed transition-opacity duration-300 ${isSelected ? 'text-brand-text/50 opacity-100' : 'text-brand-muted/40 opacity-0 group-hover:opacity-100'
                          }`}>
