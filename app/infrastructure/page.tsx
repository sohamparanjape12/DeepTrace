...
const SECTIONS = [
  ...
  {
    id: 'gemini',
    title: 'Gemini Rights Engine',
    description: 'Powered by advanced multimodal models, our rights engine autonomously audits every detected match against complex licensing structures.',
    icon: Brain,
    details: [
  ...
  },
  {
    ...
    description: 'Direct integration with major registrars, CDNs, and legal platforms to execute takedowns and licensing settlements.',
    icon: Zap,
    details: [
      'Pre-verified legal standing',
      'Direct API hooks to hosting providers',
      'Automated DMCA/Notice-and-Takedown',
      ...
      <div className="max-w-6xl mx-auto px-6 py-12 space-y-24">
        ...
        <div className="space-y-8 max-w-4xl">
          <div className="space-y-3">
            <h1 className="text-5xl md:text-8xl font-display font-black uppercase tracking-tight text-zinc-950 dark:text-white leading-[0.85]">
              The Engine of <br />
          ...
          {SECTIONS.map((section) => (
            ...
            >
              <div className="flex justify-between items-start">
                ...
                <span className="text-[10px] font-black uppercase tracking-[0.3em] text-brand-accent opacity-0 group-hover:opacity-100 transition-opacity">Active System</span>
              </div>

              <div className="space-y-4">
                <h2 className="text-4xl font-display font-black uppercase tracking-tight text-zinc-950 dark:text-white leading-none">
                  {section.title}
                </h2>
              ...
              <ul className="grid grid-cols-1 gap-4 pt-4">
                {section.details.map((detail) => (
                  <li key={detail} className="flex items-center gap-4 group/item">
                    ...
                    <span className="text-[12px] font-black uppercase tracking-widest text-zinc-400 dark:text-white/20 group-hover/item:text-zinc-950 dark:group-hover/item:text-white transition-colors">
                      {detail}
                    </span>
                  </li>
                ))}
        ...
        <div className="relative py-24 px-12 bg-zinc-950 dark:bg-black rounded-[3rem] overflow-hidden border border-zinc-800 text-center space-y-16">
          ...
          <div className="space-y-4">
            <h3 className="text-[11px] font-black uppercase tracking-[0.4em] text-brand-accent">Architecture Stack</h3>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 opacity-40 hover:opacity-100 transition-opacity duration-700">
            {[
              { label: 'Cloudflare Workers', icon: Zap },
              { icon: Database, label: 'Postgres Vector' },
              { icon: BarChart3, label: 'TimescaleDB' },
              { icon: Lock, label: 'Vault Security' },
            ].map((item) => (
